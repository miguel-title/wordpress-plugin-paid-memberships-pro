...
CREATE TABLE `wp_pmpro_membership_levels` (
  ...
  `expiration_number` int(10) unsigned NOT NULL,
  `expiration_period` enum('Day','Week','Month','Year') NOT NULL,
  `anual_refund` decimal(18, 8) NOT NULL DEFAULT '0.00',
  PRIMARY KEY (`id`),
...
CREATE TABLE `wp_pmpro_membership_orders` (
  ...
  `petbirthdate` varchar(127) NOT NULL,
  `petsex` varchar(15) NOT NULL,
  `petspecies` varchar(127) NOT NULL,
  `petbreed` varchar(127) NOT NULL,
  'bcpf' varchar(127) NOT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `code` (`code`),
