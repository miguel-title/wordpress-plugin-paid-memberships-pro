...
if ( ! function_exists( 'sornot' ) ) {
	function sornot( $t, $n ) {
		if ( $n == 1 ) {
			return __($t, 'paid-memberships-pro');
		} else {
			return __( $t . 's', 'paid-memberships-pro' );
		}
	}
}
...
////////////////////////////////////////////////
	...
	if ( ! empty( $level ) ) {
		...
		if ( is_array( $level ) ) {
			...
			// 		(`user_id`, `membership_id`, `code_id`, `initial_payment`, `billing_amount`, `cycle_number`, `cycle_period`, `billing_limit`, `trial_amount`, `trial_limit`, `startdate`, `enddate`, `petname`, `petbirthdate`, `petsex`, `petspecies`, `petbreed`)
			// 		VALUES
			// 		( %d, %d, %d, %s, %s, %d, %s, %d, %s, %d, %s, %s, %s, %s, %d, %s, %s )",
			...
			$sql = "INSERT INTO {$wpdb->pmpro_memberships_users}
					(`user_id`, `membership_id`, `code_id`, `initial_payment`, `billing_amount`, `cycle_number`, `cycle_period`, `billing_limit`, `trial_amount`, `trial_limit`, `startdate`, `enddate`, `petname`, `petbirthdate`, `petsex`, `petspecies`, `petbreed`)	VALUES";
			$petcount = count(explode(',', $level['petname']));
			for($petindex = 0; $petindex < $petcount; $petindex++){
				$values[] = $wpdb->prepare( "(%d, %d, %d, %s, %s, %d, %s, %d, %s, %d, %s, %s, %s, %s, %s, %s, %s)", $level['user_id'], $level['membership_id'], $level['code_id'], $level['initial_payment'], $level['billing_amount'], $level['cycle_number'], $level['cycle_period'], $level['billing_limit'], $level['trial_amount'], $level['trial_limit'], $level['startdate'], $level['enddate'], explode(',', $level['petname'])[$petindex], explode(',', $level['petbirthdate'])[$petindex], explode(',', $level['petsex'])[$petindex], explode(',', $level['petspecies'])[$petindex], explode(',', $level['petbreed'])[$petindex] );
			}
			$sql .= implode( ',', $values );
		} else {
			$sql = $wpdb->prepare(
