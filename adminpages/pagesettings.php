...
if (!empty($_REQUEST['savesettings'])) {
    ...
    pmpro_setOption("consult_page_id", NULL, 'intval');
    pmpro_setOption("refund_page_id", NULL, 'intval');
    pmpro_setOption("requestnewrefund_page_id", NULL, 'intval');
    ...
    $pmpro_pages['consult'] = pmpro_getOption('consult_page_id');
    $pmpro_pages['refund'] = pmpro_getOption('refund_page_id');
    $pmpro_pages['requestnewrefund'] = pmpro_getOption('requestnewrefund_page_id');
...
if (!empty($_REQUEST['createpages'])) {
    ...
    if(empty($_REQUEST['page_name'])) {
        ...
        $pages['consult'] == __('Membership Consult', 'paid-memberships-pro' );
        $pages['refund'] == __('Membership Refund', 'paid-memberships-pro' );
...
?>
    ...
    <form action="<?php echo admin_url('admin.php?page=pmpro-pagesettings');?>" method="post" enctype="multipart/form-data">
        ...
        <table class="form-table">
            ...
            <tr>
				<th scope="row" valign="top">
					<label for="consult_page_id">página de Consulte:</label>
				</th>
				<td>
					<?php
						wp_dropdown_pages(
							array(
								'name' => 'consult_page_id',
								'show_option_none' => '-- ' . __('Use WordPress Default', 'paid-memberships-pro') . ' --',
								'selected' => $pmpro_pages['consult'], 'post_types' => $post_types
							)
						);
					?>
                    
					<?php if ( ! empty( $pmpro_pages['consult'] ) ) { ?>
                        <a target="_blank" href="post.php?post=<?php echo $pmpro_pages['consult'] ?>&action=edit"
                            class="button button-secondary pmpro_page_consult"><?php _e('edit page', 'paid-memberships-pro' ); ?></a>
                        &nbsp;
                        <a target="_blank" href="<?php echo get_permalink($pmpro_pages['consult']); ?>"
                        class="button button-secondary pmpro_page_view"><?php _e('view page', 'paid-memberships-pro' ); ?></a>
                        <?php } elseif ( empty( pmpro_getOption( 'login_page_generated' ) ) ) { ?>
						&nbsp;
						<a href="<?php echo wp_nonce_url( add_query_arg( array( 'page' => 'pmpro-pagesettings', 'createpages' => 1, 'page_name' => esc_attr( 'login' )   ), admin_url('admin.php') ), 'createpages', 'pmpro_pagesettings_nonce' ); ?>"><?php _e('Generate Page', 'paid-memberships-pro' ); ?></a>
					...
					<p class="description"><?php printf( esc_html__('Include the shortcode %s or the Member Consult block.', 'paid-memberships-pro' ), '[pmpro_consult]' ); ?></p>
			    </td>
			</tr>

            <tr>
				<th scope="row" valign="top">
					<label for="refund_page_id">Página de reembolso:</label>
				</th>
				<td>
					<?php
						wp_dropdown_pages(
							array(
								'name' => 'refund_page_id',
								'show_option_none' => '-- ' . __('Use WordPress Default', 'paid-memberships-pro') . ' --',
								'selected' => $pmpro_pages['refund'], 'post_types' => $post_types
							)
						);
					?>
                    
					<?php if ( ! empty( $pmpro_pages['refund'] ) ) { ?>
                        <a target="_blank" href="post.php?post=<?php echo $pmpro_pages['refund'] ?>&action=edit"
                            class="button button-secondary pmpro_page_refund"><?php _e('edit page', 'paid-memberships-pro' ); ?></a>
                        &nbsp;
                        <a target="_blank" href="<?php echo get_permalink($pmpro_pages['refund']); ?>"
                        class="button button-secondary pmpro_page_view"><?php _e('view page', 'paid-memberships-pro' ); ?></a>
                        <?php } elseif ( empty( pmpro_getOption( 'login_page_generated' ) ) ) { ?>
						&nbsp;
						<a href="<?php echo wp_nonce_url( add_query_arg( array( 'page' => 'pmpro-pagesettings', 'createpages' => 1, 'page_name' => esc_attr( 'login' )   ), admin_url('admin.php') ), 'createpages', 'pmpro_pagesettings_nonce' ); ?>"><?php _e('Generate Page', 'paid-memberships-pro' ); ?></a>
					...
					<p class="description"><?php printf( esc_html__('Include the shortcode %s or the Member Refund block.', 'paid-memberships-pro' ), '[pmpro_refund]' ); ?></p>
			    </td>
			</tr>
            ...
            <tr>
				<th scope="row" valign="top">
					<label for="requestnewrefund_page_id">Página de Solicitar Novo Reembolso:</label>
				</th>
				<td>
					<?php
						wp_dropdown_pages(
							array(
								'name' => 'requestnewrefund_page_id',
								'show_option_none' => '-- ' . __('Use WordPress Default', 'paid-memberships-pro') . ' --',
								'selected' => $pmpro_pages['requestnewrefund'], 'post_types' => $post_types
							)
						);
					?>
                    
					<?php if ( ! empty( $pmpro_pages['requestnewrefund'] ) ) { ?>
                        <a target="_blank" href="post.php?post=<?php echo $pmpro_pages['requestnewrefund'] ?>&action=edit"
                            class="button button-secondary pmpro_page_refund"><?php _e('edit page', 'paid-memberships-pro' ); ?></a>
                        &nbsp;
                        <a target="_blank" href="<?php echo get_permalink($pmpro_pages['requestnewrefund']); ?>"
                        class="button button-secondary pmpro_page_view"><?php _e('view page', 'paid-memberships-pro' ); ?></a>
                        <?php } elseif ( empty( pmpro_getOption( 'login_page_generated' ) ) ) { ?>
						&nbsp;
						<a href="<?php echo wp_nonce_url( add_query_arg( array( 'page' => 'pmpro-pagesettings', 'createpages' => 1, 'page_name' => esc_attr( 'login' )   ), admin_url('admin.php') ), 'createpages', 'pmpro_pagesettings_nonce' ); ?>"><?php _e('Generate Page', 'paid-memberships-pro' ); ?></a>
					...
					<p class="description"><?php printf( esc_html__('Include the shortcode %s or the Member Request new refund block.', 'paid-memberships-pro' ), '[pmpro_requestnewrefund]' ); ?></p>
			    </td>
			</tr>
