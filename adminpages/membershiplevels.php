<?php
	...
	if($action == "save_membershiplevel") {
		...
		$m1_anual_refund = intval($_REQUEST['anual_refund']);
	...
	<div>
		...
		<table class="form-table">
			<tbody>
				...
				<tr>
					<th scope="row" valign="top"><label>Reembolso Anual</label></th>
					<td>
					<?php
						if(pmpro_getCurrencyPosition() == "left")
							echo $pmpro_currency_symbol;
					?>
					<input id="anual_refund" name='anual_refund' type='text' value="<?php echo esc_attr(pmpro_filter_price_for_text_field($level->anual_refund));?>"></td>
				</tr>
