...
global $username, $password, $password2, $bfirstname, $blastname, $baddress1, $baddress2, $bcity, $bstate, $bzipcode, $bcountry, $bphone, $bemail, $bconfirmemail, $CardType, $AccountNumber, $ExpirationMonth, $ExpirationYear
		,$bpetname, $bpetbirthdate, $bpetsex, $bpetspecies, $bpetbreed, $bcpf;

if ( isset( $_REQUEST['petname'] ) ){
	$bpetname = sanitize_text_field( stripslashes( $_REQUEST['petname'] ) );
}	else {
	$bpetname = "";
}
if ( isset( $_REQUEST['birthdate'] ) ){
	$bpetbirthdate = sanitize_text_field( stripslashes( $_REQUEST['birthdate'] ) );
}	else {
	$bpetbirthdate = "";
}
if ( isset( $_REQUEST['genre'] ) ){
	$bpetsex = sanitize_text_field( stripslashes( $_REQUEST['genre'] ) );
}	else {
	$bpetsex = "";
}
if ( isset( $_REQUEST['species'] ) ){
	$bpetspecies = sanitize_text_field( stripslashes( $_REQUEST['species'] ) );
}	else {
	$bpetspecies = "";
}
if (isset( $_REQUEST['bcpf'])){
	$bcpf = sanitize_text_field( stripslashes( $_REQUEST['bcpf'] ) );
}	else{
	$bcpf = "";
}
if ( isset( $_REQUEST['breed'] ) ){
	$bpetbreed = sanitize_text_field( stripslashes( $_REQUEST['breed'] ) );
}	else {
	$bpetbreed = "";
} 
...
$pmpro_required_billing_fields = array(
	'bpetname'		  => $bpetname,
	'bpetbirthdate'   => $bpetbirthdate,
	'bpetsex'		  => $bpetsex,
	'bpetspecies'	  => $bpetspecies,
	'bpetbreed'		  => $bpetbreed,
		...
		foreach ( $pmpro_required_billing_fields as $key => $field ) {
			if ( ! $field ) {
				...
				if ($skip_account_fields){
					if ($key == 'bfirstname' || $key == 'blastname' || $key == 'bphone' || $key == 'bcpf' || $key == 'bpetname' || $key == 'bpetbirthdate' ||  $key == 'bpetsex' ||  $key == 'bpetspecies' ||  $key == 'bpetbreed' ){
						continue;
					}	
				}
...
if ( ! empty( $pmpro_confirmed ) ) {
	...
	if ( ! empty( $user_id ) && ! is_wp_error( $user_id ) ) {
		...
		$custom_level = array(
			...
			'enddate'         => $enddate,
			'petname'		  => $bpetname,
			'petbirthdate'    => $bpetbirthdate,
			'petsex'          => $bpetsex,
			'petspecies'      => $bpetspecies,
			'petbreed'        => $bpetbreed
		);
		if ( pmpro_changeMembershipLevel( $custom_level, $user_id, 'active' ) ) {
			//we're good
