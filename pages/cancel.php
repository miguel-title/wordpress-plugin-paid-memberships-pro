...
<div id="pmpro_cancel" class="<?php echo pmpro_get_element_class( 'pmpro_cancel_wrap', 'pmpro_cancel' ); ?>">
	...
	<?php
		if(!$pmpro_confirm)
		{
			...
			?>
			<textarea name="comment" id="inp_comment" rows="10" cols="100" style="margin-bottom:30px;" placeholder="Descreva o motivo do cancelamento do plano."></textarea>
			<div class="<?php echo pmpro_get_element_class( 'pmpro_actionlinks' ); ?>">
				<a class="<?php echo pmpro_get_element_class( 'pmpro_btn pmpro_btn-submit pmpro_yeslink yeslink', 'pmpro_btn-submit' ); ?>" href="<?php echo pmpro_url("cancel", "?levelstocancel=" . esc_attr($_REQUEST['levelstocancel']) . "&confirm=true&comment=")?>"><?php _e('Yes, cancel this membership', 'paid-memberships-pro' );?></a>
...
<script>
	jQuery(document).ready(function ($) {
		$("a.yeslink").click(function(e){
			e.preventDefault();
			var oldhref = $("a.yeslink").attr("href");
			var newhref = oldhref + "=" + $("#inp_comment").val();
