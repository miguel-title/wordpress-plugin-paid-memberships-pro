	...
	<div id="pmpro_account">
		<?php if(in_array('membership', $sections) || in_array('memberships', $sections)) { ?>
			<div id="pmpro_account-membership" class="<?php echo pmpro_get_element_class( 'pmpro_box', 'pmpro_account-membership' ); ?>">
				...
				<table class="<?php echo pmpro_get_element_class( 'pmpro_table' ); ?>" width="100%" cellpadding="0" cellspacing="0" border="0">
					...
					<tbody>
						...
						<tr>
							...
							?>
							<tr>
								<td class="<?php echo pmpro_get_element_class( 'pmpro_account-membership-petname' ); ?>">
									<?php
										$pmpro_member_petname_action_link = sprintf( '<span id="pmpro_petname_actionlink" href="%s">%s</span>', esc_url( add_query_arg( array('id' => $level->subscription_id, 'level' => $level->id), pmpro_url( 'consult', '', 'https' ) ) ), $level->petname );
										$allowed_html = array(
											'a' => array (
												'class' => array(),
												'href' => array(),
										...
										echo wp_kses( $pmpro_member_petname_action_link, $allowed_html );
									?>
								</td>
				...
				<div class="<?php echo pmpro_get_element_class( 'pmpro_actionlinks' ); ?>">
					<a id="pmpro_actionlink-levels" href="<?php echo pmpro_url("levels")?>">Adicione mais um pet</a>
				</div>
		...
		<?php if(in_array('profile', $sections)) { ?>
			<div id="pmpro_account-profile" class="<?php echo pmpro_get_element_class( 'pmpro_box', 'pmpro_account-profile' ); ?>">
				...
				<?php } ?>
				<?php 
					$userdatas = $wpdb->get_results("SELECT billing_street, billing_city, billing_country, accountnumber, billing_state, billing_phone FROM $wpdb->pmpro_membership_orders WHERE user_id = '$current_user->ID' limit 1");
				?>
				<ul>
					...
					<?php foreach ($userdatas as $userdata){ ?>
						<li><strong>CPF:</strong> <?php $userdata->bcpf; ?></li>
						<li><strong>Endereço:</strong> <?php echo $userdata->billing_street . ' ' . $userdata->billing_city . ' ' . $userdata->billing_country; ?>&nbsp;&nbsp;&nbsp;&nbsp;<strong>Número:</strong> <?php echo $userdata->accountnumber; ?>&nbsp;&nbsp;&nbsp;&nbsp;<strong>Cidade:</strong> <?php echo $userdata->billing_city; ?>&nbsp;&nbsp;&nbsp;&nbsp;<strong>Estado:</strong> <?php echo $userdata->billing_state; ?></li>
						<li><strong>Telefone:</strong> <?php echo $userdata->billing_phone; ?></li>
					<?php } ?>
...
// 						}
			...
			<div id="pmpro_account-pet" class="<?php echo pmpro_get_element_class( 'pmpro_box', 'pmpro_account-profile' ); ?>">
				<h3>Pet</h3>
				<?php 
					$petdatas = $wpdb->get_results("SELECT petname, petbirthdate, petsex, petspecies, petbreed FROM $wpdb->pmpro_memberships_users WHERE user_id = '$current_user->ID' and status = 'active'");
				?>
				<ul>
				<? foreach($petdatas as $petdata){?>
					<li><strong>Nome do Pet:</strong> <?php echo $petdata->petname; ?></li>
					<li><strong>Data de Nascimento:</strong> <?php echo $petdata->petbirthdate; ?></li>
					<li><strong>Sexo:</strong> <?php echo $petdata->petsex; ?></li>
					<li><strong>Espécie:</strong> <?php echo $petdata->petspecies; ?></li>
					<li><strong>Raça:</strong> <?php echo $petdata->petbreed; ?></li>
					<br/>
				<?}?>
				</ul>
			</div>
